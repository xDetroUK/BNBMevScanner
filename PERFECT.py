...
def calculate_frontrun_gas_price(victim_gas_price, overpayment_percentage=0.2):
    """
...
def calculate_slippage(amount_in, amount_out_min, path):
    try:
        # Fetch expected amounts using the DEX router
        ...
        slippage = ((expected_output - amount_out_min) / expected_output) * 100
...
def calculate_optimal_dx(victim_input, min_tokens_out, reserve_x, reserve_y, dex_fee):
    try:
        if min_tokens_out <= 0 or reserve_x <= 0 or reserve_y <= 0:
            return None

        # Calculate maximum dx
        numerator = victim_input
        denominator = (reserve_y / min_tokens_out) * dex_fee - 1

        if denominator <= 0:
            return None

        # Total dx before affecting victim's output
        total_dx = numerator / denominator

        # Use 85% of maximum possible dx for safety
        return 0.85 * total_dx if total_dx > 0 else None
    except Exception as e:
        return None
    ...
    try:
        ...
        if dx is not None:
            ...
            try:
                ...
                frontrun_gas_price = calculate_frontrun_gas_price(victim_gas_price, overpayment_percentage=0.75)  # Overpay by 75%
